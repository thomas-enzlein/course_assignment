...
test_that("optimize_courses works with enforce_survival", {
  # Simple mock data
  students <- data.frame(
    student_id = c("S1", "S2"),
    first_choice = c("C1", "C2"),
    second_choice = c("C2", "C1"),
    third_choice = c("C2", "C1"),
    tie_breaker = c(0.01, 0.02),
    stringsAsFactors = FALSE
  )
  courses <- data.frame(
    course_id = c("C1", "C2"),
  ...
  res <- optimize_courses(students, courses, enforce_survival = TRUE)
  expect_true(res$status %in% c("optimal", "success"))
  expect_equal(nrow(res$assignments), 2)
})
